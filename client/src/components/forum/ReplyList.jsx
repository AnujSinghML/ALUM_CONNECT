// client\src\components\forum\ReplyList.jsx
...
const Reply = ({ 
  ...
  const handleReplyVote = async (replyId, voteType) => {
    ...
    try {
      ...
      // Update the replies through the parent component
      const updatedVoteCount = response.data.voteCount;
      setReplies(prevReplies => updateReplyVotes(prevReplies, replyId, updatedVoteCount));
    } catch (error) {
  ...
  const updateReplyVotes = (replies, replyId, newVoteCount) => {
    return replies.map(reply => {
      if (reply._id === replyId) {
        return { ...reply, voteCount: newVoteCount };
      }
      if (reply.childReplies && reply.childReplies.length > 0) {
        return {
          ...reply,
          childReplies: updateReplyVotes(reply.childReplies, replyId, newVoteCount)
        };
      }
      return reply;
    });
  ...
  const handleDeleteReply = async (replyId) => {
    if (!window.confirm("Are you sure you want to delete this reply? This will also delete all replies to this comment.")) return;
          ...
          <>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                ...
                {/* Toggle for thread expansion if there are child replies */}
